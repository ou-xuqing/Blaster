...
# --- Priority 2: Read .uproject EngineAssociation 鈫?Windows Registry ---
if (-not $UEPython) {
    ...
    foreach ($upf in $uprojectFiles) {
        try {
            ...
            if ($engineVer) {
                ...
                # Try HKLM installed engine (e.g. "5.7" 鈫?HKLM\SOFTWARE\EpicGames\Unreal Engine\5.7)
...
# Helper: run pip with a process timeout.
function Invoke-PipInstall {
    ...
    # Let pip print directly to the console.
    ...
    if (-not $exited) {
        ...
        return 124   # 浠?Linux timeout 閫€鍑虹爜
    }

    return $proc.ExitCode
}

# Prefer offline vendor wheels when present.
